...
function setup() {
  ...
  ground = createSprite(camera.position.x,180,400,20);
  ground.addImage("ground",groundImage);
  ground.x = ground.width /2;
  ground.velocityX = -(6 + 3*score/100);
  ground.visible= false;

  gameOver = createSprite(camera.position.x + 80, 30);
  gameOver.addImage(gameOverImg);
  
  restart = createSprite(camera.position.x + 80, 70);
